...
const {optionsForRedis} = require('./common/config');

const secret = 'secret-key';

const redisClient = redis.createClient(optionsForRedis);
(async () => {
        ...
        if (rc) {
            ...
            jwt.verify(refreshToken, secret, async (err, payload) => {
                if (err) {
                    ...
                    res.cookie('access-token', jwt.sign({id: payload.id, username: payload.username}, secret, {expiresIn: '10m'}));
                    res.cookie('refresh-token', jwt.sign({id: payload.id, username: payload.username}, secret, {expiresIn: '24h'}));
...
app.post('/login', async (req, res) => {
    ...
    if (user?.password === password) {
        res.cookie('access-token', jwt.sign({id: user.id, username: user.username}, secret, {expiresIn: '10m'}), {httpOnly: true});
        res.cookie('refresh-token', jwt.sign({id: user.id, username: user.username}, secret, {expiresIn: '24h'}));
        res.redirect('/resource');
    } else {
